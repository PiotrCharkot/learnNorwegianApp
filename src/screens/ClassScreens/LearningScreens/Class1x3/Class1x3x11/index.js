...
const correctAnswers = ['Han', 'snakket']; // array with correct words in line up to the word that goes in gap. exapmle ==> const correctAnswers = ['Jeg', 'trenger'];

const indexOfGaps = [1] // array with index of a gap in text
const indexOfText = [0, 2, 3] // array with indexes of text

const Class1x3x11 = ({ route }) => {  //screen name

    const {userPoints, latestScreen, comeBackRoute, allScreensNum, savedLang} = route.params
    
    const [movingDraggable, setMovingDraggable] = useState(null);
    const [releaseDraggable, setReleaseDraggable] = useState(null);
    const [words, setWords] = useState(['Han' ,'            ', 'med meg', 'i går.','!!!' , 'snakk', 'snakke', 'snakket', 'å snakket', 'snakkte' ]); // arr with word in order, after '!!!' comes word to choose from
  ...
  return (
    <View style={styles.mainContainer}>
      <ProgressBar screenNum={currentScreen} totalLenghtNum={allScreensNum} latestScreen={latestScreenDone} comeBack={comeBackRoute}/>
        <View style={styles.body}>
            ...
            <View style={styles.topView}>
                <Text style={styles.questionText}>Choose correct verb form for past tense. Drag and drop it into the gap.</Text>
                <Text style={styles.textBody}>(He <Text style={styles.textColor}>spoke</Text> to me yesterday.)</Text>
            </View>
      ...
      <View style={styles.bottomBarContainer}>
        ...
        linkNext={'Class1x3x12'} //link to next screen
        linkPrevious={'Class1x3x10'} //link to previous screen
...
export default Class1x3x11 //name for export
