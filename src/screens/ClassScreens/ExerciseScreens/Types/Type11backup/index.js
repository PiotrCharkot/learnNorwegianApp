...
const Type11opening = ({ route }) => {
  ...
  const sendForTranscription = async () => {
    ...
    const newPath = FileSystem.documentDirectory + 'recording.wav';
    await FileSystem.copyAsync({ from: recordedUri, to: newPath });



    const uploadResult = await FileSystem.uploadAsync(
        'https://us-central1-norapp-69bd4.cloudfunctions.net/api/transcribeAudio',
        newPath,
        {
          httpMethod: 'POST',
          uploadType: FileSystem.FileSystemUploadType.MULTIPART,
          fieldName: 'file',           // must match the field name for multer.single('file')
          mimeType: 'audio/wav',       // or your actual MIME type
          headers: {
            // Optional, e.g. if you need Authorization or something
          },
        }
    );
